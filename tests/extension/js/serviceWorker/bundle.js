...
var REXChatGoogleAISpider = class extends REXSpider {
  constructor() {
    super(...arguments);
  ...
  checkLogin() {
    return new Promise((resolve) => {
      ...
      fetch(this.loginUrl()).then((response) => {
        if (response.ok) {
    ...
    return new Promise((resolve) => {
      ...
      service_worker_default.handleMessage(fetchLastSync, this, (response) => {
        ...
        service_worker_default.handleMessage(storeMessage, this, (response2) => {
          this.syncing = true;
          fetch(this.fetchInitialUrls()[0]).then((response3) => {
            if (response3.ok) {
              response3.text().then((rawResponse) => {
                console.log(`[rex-spider-google-ai] Fetched list payload (${response3.status}: ${response3.statusText}):`);
  ...
  parseListResponse(rawContent) {
    ...
    return new Promise((resolve) => {
      const nextConvo = () => {
        ...
        this.parseConversation(next).then((parsedConvo) => {
  ...
  parseConversationOff(conversationJson) {
    return new Promise((resolve) => {
      console.log(`[rex-spider-chatgpt] parseConversation:`);
      console.log(conversationJson);
      const firstWhen = new Date(conversationJson["create_time"] * 1e3);
      const latestDate = firstWhen;
      const firstWhenString = new DateString(conversationJson["create_time"]);
      const conversation = {
        turns: [],
        platform: "chatgpt",
        ...
        metadata: conversationJson
        // TODO: Pull out so only populated on debug=true
      };
          ...
          if (turnJson !== void 0) {
            ...
            if (turnJson.message !== null) {
              if (turnJson["create_time"] !== null) {
                createTime = new DateString(`${turnJson["create_time"] * 1e3}`);
              }
              const turn = {
              ...
              if (turnJson.message.content.parts !== void 0) {
                turn["content*"] = turnJson.message.content.parts.join("\n");
              } else if (turnJson.message.content.text !== void 0) {
                turn["content*"] = turnJson.message.content.text;
              }
              if (turnJson.metadata !== void 0) {
                if (turnJson.metadata["search_result_groups"] !== void 0) {
                  const search = {
                    platform: "chatgpt",
                    "query*": "?",
                    type: "web",
                  ...
                  for (const searchGroup of turnJson.metadata["search_result_groups"]) {
                    for (const entry of searchGroup.entries) {
                      search.results.push({
                        title: entry["title"],
                        url: entry["url"],
                ...
                if (turnJson.metadata["content_references"] !== void 0) {
                  turn.citations = [];
      ...
      service_worker_default.handleMessage(message, this, (response) => {
        let timestamp = 0;
        if (response !== null) {
          timestamp = response;
        }
...
var googleAISpider = new REXChatGoogleAISpider();
service_worker_default2.registerSpider(googleAISpider);
var service_worker_default3 = googleAISpider;

// tests/src/service-worker.ts
self["rexCorePlugin"] = service_worker_default;
self["rexSpiderPlugin"] = service_worker_default2;
self["rexGoogleAIPlugin"] = service_worker_default3;
...
service_worker_default2.registerSpider(service_worker_default3);
